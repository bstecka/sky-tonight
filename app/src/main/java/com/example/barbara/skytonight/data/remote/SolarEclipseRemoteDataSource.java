...
public class SolarEclipseRemoteDataSource implements SolarEclipseDataSource {
    ...
    public void getSolarEclipses(final double latitude, final double longitude, final GetSolarEclipsesCallback callback) {
        ...
        final String url = "http://skytonight-backend.herokuapp.com/api/v1/solar-eclipses?future=1";
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < arr.length(); i++) {
                        ...
                        Date date = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).parse(object.getString("date"));
                        cal.setTime(date);
                        SolarEclipseEvent eclipseEvent = new SolarEclipseEvent(id, "Solar Eclipse", cal, type);
